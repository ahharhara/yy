...
import { getFirestore, doc, getDocFromServer, getDoc, setDoc, collection, query, where, getDocs } from 'firebase/firestore';
import { 
  getAuth, 
  GoogleAuthProvider, 
  signInWithPopup, 
...
import { APP_USERS } from '../data/initialCatalog';
...
export async function checkUserPhoneInDb(phone: string): Promise<{
  ...
  // 3. Check pre-seeded APP_USERS catalog
  const foundInCatalog = APP_USERS.find(
    (u) => normalizePhoneDigits(u.phone) === normalized
  );
  if (foundInCatalog) {
    return {
      exists: true,
