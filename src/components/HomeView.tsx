...
import { CATEGORIES, PROMO_BANNERS, HADRAMOUT_RECIPES } from '../data/initialCatalog';
...
export const HomeView: React.FC<HomeViewProps> = ({
  ...
  const handleRecipeAdd = (recipe: Recipe) => {
    ...
    setTimeout(() => setAddedRecipeId(null), 1200);
  };
  ...
  // Hadramout exclusive specials
  const hadramoutSpecials = products.filter((p) => p.category === 'hadramout-specials');
    ...
    <div className="flex-1 flex flex-col bg-[#F8F9FA] pb-12 space-y-4 overflow-x-hidden" dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <div className="bg-gradient-to-b from-[#690B24] via-[#851134] to-[#A31840] text-white pt-3 pb-5 px-4 rounded-b-[28px] shadow-lg">
        <div className="max-w-4xl mx-auto space-y-3.5">
          {/* Top Bar: Store Logo & Location & Utilities */}
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-[10px] text-rose-200/90 font-bold uppercase tracking-wider">
                  {t('appTagline', language)}
                </p>
              </div>
            </div>

            {/* Delivery badge & Address dropdown & Utilities */}
            <div className={`flex flex-col gap-1 ${isRtl ? 'items-end' : 'items-start'}`}>
              <div className="flex items-center gap-1.5">
                {/* Delivery status pill */}
                <div className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full border border-white/30 border-dashed text-[10px] font-bold text-white bg-white/10 backdrop-blur-xs">
                  <span>⚡</span>
                  <span>{t('deliveryAvailableSoon', language)}</span>
                </div>
                ...
                {onOpenNotifications && (
                  <button
                    ...
                    className="relative w-7 h-7 rounded-lg bg-white/15 hover:bg-white/25 active:scale-95 flex items-center justify-center transition-all cursor-pointer"
                    title={t('notifications', language)}
                  >
                    <Bell className="w-3.5 h-3.5 text-white" />
                    {unreadNotificationsCount > 0 && (
                      <span className="absolute -top-1 -right-1 w-3.5 h-3.5 bg-[#F5A623] text-[#0B253A] font-black text-[9px] rounded-full flex items-center justify-center shadow-xs">
                ...
                {onOpenWallet && (
                  <button
                    ...
                    className="flex items-center gap-1 px-2 py-0.5 rounded-lg bg-white/15 hover:bg-white/25 text-white text-[10px] font-bold transition-all cursor-pointer"
                    title={t('wallet', language)}
                  >
                    <Wallet className="w-3 h-3 text-[#F5A623]" />
                    <span>{formatPrice(walletBalance, 'SAR', language)}</span>
                  </button>
                )}
              </div>

              {/* Branch / Delivery Address selector dropdown button */}
              <button
                onClick={onOpenBranchPicker}
                className="flex items-center gap-1 text-[11px] font-bold text-white/90 hover:text-white transition-colors cursor-pointer"
              >
                <MapPin className="w-3.5 h-3.5 text-[#F5A623] flex-shrink-0" />
                <span className="truncate max-w-[190px] sm:max-w-xs">
                  {currentBranch?.name || (isRtl ? 'طريق سيئون - القطن، عبودة...' : 'Seiyun - Al-Qatn Road, Aboodah...')}
                </span>
      ...
      <div className="max-w-4xl mx-auto w-full px-4 space-y-5">
        ...
        {/* 7. Authentic Hadramout Specials Section */}
        <div className="bg-gradient-to-br from-amber-50 to-orange-50/50 p-4 sm:p-5 rounded-3xl border border-amber-200 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-xl bg-gradient-to-tr from-[#B45309] to-[#F5A623] text-white flex items-center justify-center shadow-xs">
                <Crown className="w-4 h-4 text-white" />
              </div>
              <div>
                <h3 className="text-sm sm:text-base font-black text-gray-900">
                  {t('hadramoutSpecialsTitle', language)}
                </h3>
                <p className="text-[11px] text-amber-800">
                  {t('hadramoutSpecialsDesc', language)}
                </p>
              </div>
            </div>
            <button
              onClick={() => onSelectCategory('hadramout-specials')}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {hadramoutSpecials.slice(0, 3).map((prod) => {
              const curr = prod.currency || 'YER';
              return (
                <div
                  key={prod.id}
                  onClick={() => onOpenProduct(prod)}
                  className="p-3 rounded-2xl bg-white border border-amber-200/80 hover:border-amber-400 shadow-xs hover:shadow-md transition-all cursor-pointer flex gap-3 items-center group"
                >
                  <img
                    src={prod.image}
                    alt={prod.name}
                  ...
                  <div className="min-w-0 flex-1">
                    <span className="text-[9px] bg-amber-100 text-amber-900 font-bold px-1.5 py-0.5 rounded">
                      {getLocalizedProductOrigin(prod.origin, prod.originEn, language)}
                    </span>
                    ...
                    <div className="flex items-center justify-between mt-1">
                      ...
                      <button
                        onClick={(e) => handleQuickAdd(prod, e)}
                        className="w-7 h-7 rounded-lg bg-emerald-50 hover:bg-[#0E8A5E] text-[#0E8A5E] hover:text-white flex items-center justify-center transition-colors cursor-pointer"
                      >
                        <Plus className="w-4 h-4" />
                      </button>
                    </div>
        ...
        {/* 8. Trending Hadramout Recipes */}
        <div className="bg-white p-4 sm:p-5 rounded-3xl border border-gray-200 shadow-sm">
          <div className="flex items-center gap-2 mb-4">
            <div className="w-8 h-8 rounded-xl bg-[#0E8A5E] text-white flex items-center justify-center">
              <ChefHat className="w-4 h-4 text-[#F5A623]" />
            </div>
            <div>
              <h3 className="text-sm sm:text-base font-black text-gray-900">
                {t('smartRecipesTitle', language)}
              </h3>
              <p className="text-[11px] text-gray-500">
                {t('smartRecipesDesc', language)}
              </p>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {HADRAMOUT_RECIPES.map((rec) => {
              const isAdded = addedRecipeId === rec.id;
              const recipeIngredientsTotal = (rec.ingredients || []).reduce(
              ...
              return (
                <div
                  key={rec.id}
                  className="rounded-2xl border border-gray-200 overflow-hidden bg-gray-50/50 flex flex-col justify-between"
                >
                  <div className="relative h-36 w-full overflow-hidden">
                    ...
                    <div className={`absolute bottom-2.5 right-3 left-3 text-white ${isRtl ? 'text-right' : 'text-left'}`}>
                      <div className="flex items-center gap-2 text-[10px] text-amber-300 font-bold mb-0.5">
                        <Clock className="w-3 h-3" />
                        <span>{isRtl ? rec.cookingTime : (rec.cookingTimeEn || rec.cookingTime)}</span>
                        <span>•</span>
                        <span>{isRtl ? rec.servings : (rec.servingsEn || rec.servings)}</span>
                      </div>
                      <h4 className="text-sm font-black drop-shadow">
                        {isRtl ? rec.title : (rec.titleEn || rec.title)}
                      </h4>
                    </div>
                  </div>
                  ...
                  <div className="p-3.5 space-y-3 flex-1 flex flex-col justify-between">
                    <p className="text-[11px] text-gray-600 leading-relaxed">
                      {isRtl ? rec.description : (rec.descriptionEn || rec.description)}
                    </p>

                    <div>
                      <span className="text-[10px] font-bold text-gray-500 block mb-1">
                        {isRtl ? `المقادير المشمولة (${(rec.ingredients || []).length} مقادير):` : `Ingredients Included (${(rec.ingredients || []).length}):`}
                      </span>
                      <div className="flex flex-wrap gap-1">
                        {(rec.ingredients || []).map((ing, i) => (
                          ...
                          >
                            {isRtl ? ing?.name : (ing?.nameEn || ing?.name || '')}
                          </span>
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="pt-2 border-t border-gray-200 flex items-center justify-between">
                      <div>
                        <span className="text-[10px] text-gray-400 block">{t('totalAmount', language)}</span>
                        <span className="text-xs font-black text-[#0E8A5E]">
                          {formatPrice(recipeIngredientsTotal, 'SAR', language)}
                        </span>
                      </div>

                      <button
                        onClick={() => handleRecipeAdd(rec)}
                        className={`py-2 px-3.5 rounded-xl font-bold text-xs flex items-center gap-1.5 shadow-md transition-all cursor-pointer ${
                          isAdded
                            ? 'bg-emerald-700 text-white'
                            : 'bg-[#0E8A5E] hover:bg-[#095B3E] active:scale-95 text-white'
                        }`}
                      >
                        {isAdded ? (
                          <>
                            <Check className="w-4 h-4 text-[#F5A623]" />
                            <span>{t('ingredientsAdded', language)}</span>
                          </>
                        ) : (
                          <>
                            <ShoppingBag className="w-4 h-4" />
                            <span>{t('addAllIngredients', language)}</span>
                          </>
                        )}
                      </button>
                    </div>
