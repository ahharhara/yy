  ...
  return (
    <div className="flex-1 bg-white flex flex-col min-h-full select-none" dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <div className="flex-1 flex flex-col divide-y divide-gray-100">
        ...
        {/* 7. تحميل وبناء تطبيق الأندرويد (APK / App Download) */}
        <button
          onClick={() => setActiveModal('apk')}
          className={`w-full px-6 py-4.5 flex items-center justify-between bg-emerald-50/40 hover:bg-emerald-50 active:bg-emerald-100/70 transition-colors ${isRtl ? 'text-right' : 'text-left'} cursor-pointer group`}
        >
          <div className="flex items-center gap-4">
            <div className="w-7 h-7 flex items-center justify-center text-[#095B3E] bg-emerald-100 rounded-lg">
              <Smartphone className="w-4 h-4" />
            </div>
            <div>
              <span className="text-[15px] font-black text-emerald-950 group-hover:text-[#095B3E] block">
                {isRtl ? 'تطبيق الجوال وتنزيل الـ APK' : 'Mobile App & APK Download'}
              </span>
              <span className="text-[11px] text-emerald-700 font-medium">
                {isRtl ? 'تثبيت فوري للهواتف وبناء حزمة أندرويد' : 'Instant phone install & Android build'}
              </span>
            </div>
          </div>
        ...
        >
          ...
          >
            ...
            <div className="space-y-3">
              ...
              <div className="p-3 bg-gray-50 rounded-2xl border border-gray-100">
                <h5 className="text-xs font-black text-gray-900 mb-1">
                  {isRtl ? 'ما هي سياسة استرجاع الخضار والفواكه؟' : 'What is the return policy for fresh produce?'}
                </h5>
                <p className="text-xs text-gray-600 leading-relaxed">
                  {isRtl 
                    ? 'نضمن جودة طازجة 100%. في حال عدم رضاك عن أي منتج طازج يمكنك استرجاعه مباشرة واسترداد قيمته.'
                    : 'We guarantee 100% freshness. If you are not satisfied with any fresh item, you can return it instantly for a full refund.'}
                </p>
              </div>
            </div>
          </div>
        ...
        >
          ...
          >
            ...
            <div className="space-y-4">
              ...
              {/* Honorary Developer Card */}
              <div className="p-3.5 rounded-2xl bg-gray-900 text-white text-xs">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-[10px] text-amber-400 font-bold">{t('honoraryDeveloper', language)}</span>
                  <span className="text-[9px] bg-white/10 font-mono px-2 py-0.5 rounded-full">v{DEVELOPER_INFO.version}</span>
                </div>
                <p className="font-black text-sm">{DEVELOPER_INFO.developerTitle}</p>
                <p className="text-[10px] text-gray-400 font-mono mt-0.5">{DEVELOPER_INFO.email}</p>
                <button
                  onClick={() => {
                    setActiveModal(null);
                    onOpenDocs();
                  }}
                  className="mt-3 w-full py-2 bg-white/10 hover:bg-white/20 rounded-xl text-white font-bold text-xs flex items-center justify-center gap-1.5 cursor-pointer"
                >
                  <FileCode2 className="w-3.5 h-3.5 text-amber-400" />
                  <span>{t('viewDocs', language)}</span>
                </button>
              </div>

              {/* Role Dashboard Switcher if available */}
              {onOpenRoleDashboard && (
                <button
                  onClick={() => {
                    setActiveModal(null);
                  ...
                  className="w-full py-2.5 rounded-xl bg-purple-50 hover:bg-purple-100 text-purple-800 font-bold text-xs flex items-center justify-center gap-2 border border-purple-200 cursor-pointer"
                >
                  <Code2 className="w-4 h-4 text-purple-600" />
                  <span>
                    {isRtl ? `فتح لوحة الصلاحيات (${currentUser?.role || 'الإدارة'})` : `Open Permissions (${currentUser?.role || 'Admin'})`}
                  </span>
                </button>
              )}

              {/* Register as Courier / Merchant */}
              {onOpenRegister && (
                <button
                  onClick={() => {
                    setActiveModal(null);
                  ...
                  className="w-full py-2.5 rounded-xl bg-blue-50 hover:bg-blue-100 text-blue-800 font-bold text-xs flex items-center justify-center gap-2 border border-blue-200 cursor-pointer"
                >
                  <Truck className="w-4 h-4 text-blue-600" />
                  <span>{isRtl ? 'التسجيل كمندوب توصيل أو تاجر معتمد' : 'Register as Courier or Merchant'}</span>
                </button>
              )}
      ...
      {/* 8. Modal: تحميل وتثبيت الـ APK (Android APK & PWA) */}
      {activeModal === 'apk' && (
        <ApkDownloadModal
          isOpen={activeModal === 'apk'}
