...
export default function App() {
  ...
  const [allUsers, setAllUsers] = useState<AppUser[]>(APP_USERS);
  ...
  const handleResetData = () => {
    setAllUsers(APP_USERS);
    setCurrentUser(APP_USERS[0]);
  ...
  const handleConfirmOrder = async (details: {
    ...
    // 1. Call Backend API to validate and create order on server
    try {
      const resp = await fetch('/api/orders/create', {
        ...
        body: JSON.stringify({
          ...
          customerPhone: userPhone || currentUser.phone,
      ...
      console.warn('Backend order creation fallback to local-first sync:', err);
    }

    // 2. Handle Wallet Ledger Deduction if applicable
    if (details.usedWallet && walletBalance > 0) {
      ...
      // Record double-entry transaction in Firestore
      dbRecordWalletTransaction({
        userId: currentUser.id,
        type: 'DEBIT',
        amount: deduction,
        reason: `سداد قيمة الطلب #${orderId}`,
        orderId,
        balanceAfter: Math.max(0, walletBalance - deduction),
      });

      // Also call server wallet endpoint for server-side ledger
      try {
        fetch('/api/wallet/transact', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            userId: currentUser.id,
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Android Device Status Bar Simulation (only in mobile frame) */}
        {isMobileFrame && !activeRoleView && (
          <div className="bg-[#095B3E] px-6 pt-2 pb-1 text-white text-[11px] font-mono flex items-center justify-between border-b border-emerald-700/30">
            <span>09:41</span>
            {/* Center Dynamic Notch / Speaker Cutout */}
            <div className="w-24 h-4 bg-[#0B253A] rounded-full mx-auto flex items-center justify-center">
              <div className="w-2.5 h-2.5 rounded-full bg-emerald-500/80 mr-3" />
              <div className="w-7 h-1 bg-gray-600 rounded-full" />
            </div>
